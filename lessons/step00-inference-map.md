...
学习过程中，不需要一开始就记住所有公式。更重要的是始终抓住一条主线：模型拿到了什么数据，当前模块改变了什么，输出又会交给谁。每一章只向前完成一段可理解的代码，左侧源码也会随正文逐步生长；当最后一块拼上去时，我们得到的不是若干孤立的层，而是一套能够真正运行的累计推理框架。

这一章先不急着写计算。我们从 Hugging Face 上真实的 [`Qwen/Qwen3-30B-A3B`](https://huggingface.co/Qwen/Qwen3-30B-A3B) 仓库出发，看看下载模型时究竟拿到了什么，再跟着一条 prompt 走完整段旅程：它怎样变成 token，怎样穿过 Attention 和 MoE，最后怎样产生下一个 token。

左侧现在还是一个空仓库。读到一个模块，它就会出现一个空文件。这一章先把整台机器的零件摆到桌上，认清它们的位置和连接方式；从下一章开始，我们再沿着数据流，需要什么，就亲手把什么填进去。
...
左侧现在已经有了完整的空文件树。它还不能运行，这很正常。这一章只需要先记住每个问题放在哪个文件里，后面再一块一块填上。
