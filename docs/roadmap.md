...
- 可用 CUDA 时比较估算值与 `torch.cuda.max_memory_allocated()`，解释分配器差异。

**验收标准**

- 完成教程中的 C1-C10 概念题并至少答对 8/10，完成 S1-S5 形状推导题并至少答对 4/5。
- 完成综合任务：先预测后运行，核对 shape ledger、查表、投影、元素数与手算/PyTorch 内存结果，并能解释所有差异。
- 能复述可控错误的根因与最小修正，包括奇数 packed INT4 的向上取整和非法 reshape；不以正式测试文件作为本周通过条件。
- 不看代码、输出或参考答案，能口述 `[B,S] -> [B,S,D] -> [B,S,H]` 数据流，并解释“权重理论体积”为什么不等于“进程峰值显存”。
