...
- 完成 `foundations/tensor_memory.py` 和第 1 周引导练习。
- 构造 `token_ids [B,S]`、`embedding_weight [V,D]`、`hidden [B,S,D]`。
- 为形状、非法维度、dtype 字节数编写 CPU 测试。
...
- 不运行代码即可正确写出三个基础张量的形状和每一维含义。
- 测试覆盖奇数个 INT4 元素及非法输入。
- 能解释“权重理论体积”为什么不等于“进程峰值显存”。
