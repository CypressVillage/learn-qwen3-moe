...
> 本文是每周投入、实现任务和验收标准组成的 **16 周实践节奏**，不是最推荐的首次概念阅读顺序。第一次学习请先完成 [LLM 推理整机导览](course/00-inference-overview.md)，再按 [主线课程](course/README.md) 的 `00 -> 08` 顺序阅读；需要推导、手算和受控错误练习时，再进入各周深入实验。

## 使用说明

每周投入 6-10 小时。主线始终坚持：先写形状，再写公式；先做微型实现，再接真实权重；先验证正确性，再优化性能。

主线课程按认知依赖组织，本文按 16 周工程实践组织。两者可以并行使用，但首次建立完整 LLM 推理地图时应以[主线课程](course/README.md)为准。
