...
`uv.lock` 由初次 `uv lock`/`uv sync` 生成，锁定供 RTX 4060 和 A10 使用的 Linux x86_64 依赖集合。后续安装和导出必须使用 `--locked`，避免命令隐式更新已提交的锁文件。`requirements.txt` 由以下命令生成，不手工修改。`--no-header` 避免在导出文件中嵌入输出路径，使不同目标路径的逐字节比较保持稳定：
...
通用依赖不按 GPU 型号拆分，也不把某个 CUDA wheel index 写死在 `requirements.txt`。当前锁文件面向 Linux x86_64，锁定 PyTorch 2.7.1；已安装 wheel 报告为 `2.7.1+cu126`，并包含 CUDA 12.6 runtime。该构建包含 Ampere 支持，在架构上适用于 A10，但这只说明依赖兼容性，不代表当前 A10 已通过验证。

CUDA 12.x 的 minor-version compatibility 至少需要 NVIDIA 525 系列或更新驱动，实际部署优先使用更新的受支持驱动。A10 只有在目标机器上确认驱动满足要求并成功运行环境检查器的 CUDA smoke test 后才能标记为已验证；检查脚本的实际结果是最终依据。
...
RTX 4060 和 A10 的 GPU 验证需分别在对应机器执行相同检查命令；当前无 GPU 的工作区不阻塞通用环境验收，也不得据此宣称 A10 已通过验证。
