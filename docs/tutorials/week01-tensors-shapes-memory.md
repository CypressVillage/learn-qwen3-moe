...
<a id="c2-answer"></a>
**C2 答案：** `dtype` 描述每个元素的编码类型，例如整数还是浮点数、位宽是多少；`device` 描述 Tensor 位于 CPU、CUDA 等哪个计算设备。相同 shape 只保证轴长度相同，不保证运算所需的类型和位置兼容：某些运算要求 dtype 合适或一致，而参与同一次普通 Tensor 运算的数据通常也必须位于同一 device。修复时应根据计算意图显式转换，而不是只看 shape。（[返回题目](#c2-question) · [返回提示](#c2-hint)）
...
<a id="c5-answer"></a>
**C5 答案：** `reshape` 在当前布局与目标 shape 兼容时可以返回共享存储的 view；不兼容时可以通过创建副本得到目标逻辑顺序，因此“总是 view”和“总是 copy”都不成立。`view` 必须在不重排数据的前提下共享当前存储，所以目标 shape 必须与当前 Tensor 的 size/stride 兼容；元素数相同只是必要条件，不是充分条件。（[返回题目](#c5-question) · [返回提示](#c5-hint)）

<a id="c6-answer"></a>
**C6 答案：** shape 给出逻辑轴及其长度，stride 给出某轴下标增加 1 时底层跨过多少个元素，`is_contiguous()` 判断当前 size/stride 是否满足所检查的连续布局。`transpose` 交换逻辑轴及对应 stride，通常不复制数据；PyTorch 仍可根据新下标和 stride 找到正确 value，但新轴顺序可能不再符合逐行连续条件，所以结果可以正确索引却是 non-contiguous。（[返回题目](#c6-question) · [返回提示](#c6-hint)）

<a id="c7-answer"></a>
**C7 答案：** 广播从尾部比较，是因为逐元素数组运算把最右侧轴作为对应的末端维度，较短 shape 只在左侧补缺失轴。每对维度在长度相等、其中一个为 1，或一侧缺失而视为 1 时兼容；任一位置冲突就失败。扩展表示同一个 value 按广播索引规则被重复读取，不要求用户先创建并填满一个更大 Tensor，尽管具体算子可以在输出中分配新数据。（[返回题目](#c7-question) · [返回提示](#c7-hint)）
...
<a id="s5-answer"></a>
**S5 答案：** 对每个 batch，左侧可看成 `[S,D] = [3,4]`，右侧是 `[D,H] = [4,5]`。内维 `4` 相等并收缩，左矩阵行/序列维 `S = 3` 保留，右矩阵列维 `H = 5` 产生，所以每个 batch 得到 `[3,5]`；再保留 matmul batch 维 `B = 2`，整体输出是 `[2,3,5]`。`torch.matmul` 原生支持前导 batch 维，会对每个 batch 的最后两个矩阵维执行乘法，因此无需先 reshape 成二维；不必要的展平还会暂时丢失清晰的 `B`、`S` 语义。（[返回题目](#s5-question) · [返回提示](#s5-hint)）
...
下一周会在本周形状与内存直觉之上认识 `torch.nn.Module`：它是组织神经网络计算与已注册状态的基本容器。**parameters** 是注册到模块中的参数状态，通常用于训练；但一个 parameter 只有在 `requires_grad=True`、产生梯度、被包含在 optimizer 中且执行优化步骤时，才会被该 optimizer 更新。冻结 parameter 仍然是 parameter。**buffers** 则按角色注册为非 parameter 状态：它们随模块移动，persistent buffer 也会进入 `state_dict`。两者的区别来自注册类别和用途，而不是简单等同于“是否会变化”。
